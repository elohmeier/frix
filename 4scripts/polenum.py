...
class SAMRDump:
    ...
    def dump(self, addr):
        ...
        if self.__username and self.__password:
            print(
                "[+] Attaching to {0} using {1}:{2}".format(
                    addr, self.__username, self.__password
                )
            )
        ...
        for protocol in self.__protocols:
            ...
            rpctransport = transport.SMBTransport(
                addr, port, r"\samr", self.__username, self.__password
            )
    ...
    def __fetchList(self, rpctransport):
        ...
        re = samr.hSamrQueryInformationDomain2(
            dce, domainHandle=domainHandle, domainInformationClass=domain_passwd
        )
        ...
        re = samr.hSamrQueryInformationDomain2(
            dce, domainHandle=domainHandle, domainInformationClass=domain_lockout
        )
        ...
        re = samr.hSamrQueryInformationDomain2(
            dce, domainHandle=domainHandle, domainInformationClass=domain_logoff
        )
    ...
    def __pretty_print(self):
        ...
        print(
            "\t[+] Reset Account Lockout Counter: {0}".format(
...
def main():
    ...
    parser.add_argument(
        "--protocols", nargs="*", help=str(list(SAMRDump.KNOWN_PROTOCOLS.keys()))
    )
